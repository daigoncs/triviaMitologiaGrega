# 🏛️ Trivia — Mitologia Grega
 
Quiz interativo sobre mitologia grega com navegação por hash, timer por questão e sistema de vidas.

![Badge Refatorando](http://img.shields.io/static/v1?label=STATUS&message=CONSTRUINDO...&color=GREEN&style=for-the-badge)
 
---
 
## 📱 Protótipo
 
| Home | Welcome | Quiz | Resultado |
|------|---------|------|-----------|
| Tela inicial com logo e botão Entrar | Boas-vindas com nome do jogador | Questão com timer e vidas | Pontuação final |

## ✨ Funcionalidades
 
- Navegação entre telas via `window.location.hash` sem recarregar a página
- Timer regressivo de 15 segundos por questão
- Feedback visual imediato: resposta certa (verde) e errada (vermelho)
- Pontuação exibida ao final com opção de jogar novamente

## 🛠️ Tecnologias
 
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="30" alt="html5 logo"  /> <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="30" alt="css logo"  /> <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="30" alt="javascript logo"  /> <img width="12" />
 
---
 
## 👩‍💻 Autora
 
Desenvolvido por **Daiane Gonçalves**  
[GitHub](https://github.com/daigoncs) · [LinkedIn](https://www.linkedin.com/in/daianeegoncs/)
